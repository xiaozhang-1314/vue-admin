...
let CommonUtil = {
    ...
    date:{
        ...
        GMTToStr:function(val){
           if(!val)return '';
           if(typeof val==='number')return this.dateTime(new Date(val));
           return this.dateTime(Date.parse(val));
        },
        strToTimeStamp:function(val){
            if(!val)return '';
            if(typeof val==='number')return val;
            return Date.parse(val).getTime();
         },
        //当前月的第一天
